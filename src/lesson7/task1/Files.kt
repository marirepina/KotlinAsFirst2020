 ...
 */
fun markdownToHtmlSimple(inputName: String, outputName: String): String = TODO()
    ...
    File(outputName).printWriter().use {
        ...
        res.forEachLine { st ->
            ...
            if (previousSum < sum) {
                ...
                it.println("</li>")
                val ost = previousSum - sum
                for (j in 1..ost)
                    it.println(resList.removeLast() + "\n</li>")
                it.println("<li>")
            }
            it.println(st.replace(Regex("""^((\s*\d*\.)|^(\s*\*))\s*""")) { "" })
            previousSum = sum
        }
        while (resList.isNotEmpty())
            it.println("</li>\n" + resList.removeLast())
        it.print("</p>\n" + "</body>\n" + "</html>")
    }
}
